  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        isScrolled
          ? 'bg-noir-900/80 backdrop-blur-xl border-b border-gold-400/10 py-4'
          : 'bg-transparent py-6'
      }`}
    >
      <div className="container mx-auto px-6 lg:px-8">
        <nav className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <div className={`hidden lg:flex items-center gap-10 ${isRTL ? 'flex-row-reverse' : ''}`}>
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="relative text-noir-300 hover:text-gold-400 transition-colors duration-300
          ...
          <button
            ...
            className="lg:hidden p-2 text-noir-300 hover:text-gold-400 transition-colors"
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="lg:hidden bg-noir-800/95 backdrop-blur-xl border-t border-gold-400/10"
          >
              ...
              <div className={`flex flex-col gap-6 ${isRTL ? 'items-end' : 'items-start'}`}>
                {navLinks.map((link) => (
                  <Link
                    ...
                    className="text-noir-200 hover:text-gold-400 transition-colors text-lg font-display"
                  >
                    {link.label}
                  </Link>
                ))}
                <div className={`flex items-center gap-4 pt-4 border-t border-noir-700 w-full ${isRTL ? 'flex-row-reverse justify-end' : 'justify-start'}`}>
                  <LanguageSwitcher locale={locale} />
                  <Link
                    href="#contact"
                    onClick={() => setIsMobileMenuOpen(false)}
